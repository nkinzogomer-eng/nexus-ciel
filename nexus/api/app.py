...
@app.get("/policy")
async def policy():
    return {"source": router.policy.source_path, "version": router.policy.version, "schema_version": router.policy.schema_version, "confidence_threshold": router.policy.confidence_threshold, "stages": list(router.policy.stages), "owner": router.policy.owner, "router_access": router.policy.router_access}
@app.get("/health")
